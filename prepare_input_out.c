...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/10/30 11:56:33 by eturbat           #+#    #+#             */
/*   Updated: 2022/10/30 11:58:31 by eturbat          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

int	prepare_in_hered(t_mini *data, int process_id, int redir_type, char *end)
{
	char	*filename;
	bool	do_eval;
	char	*process_id_str;
...
int	prepare_input(t_mini *data, int process_id, char *filename)
{
...
int	prepare_output(t_mini *data, int process_id, int redir_type, char *filename)
{
...
int	prepare_redir(t_mini *data, int process_id, int redir_type, char *redir)
{
	if (!redir)
		return (0);
...
int	prepare_redirs(t_mini *data, int process_id)
{
	...
	i = 0;
	while ((size_t) i < data->process[process_id].redirs.lenght / 2)
	{
